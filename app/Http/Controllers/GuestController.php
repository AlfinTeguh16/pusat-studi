...
class GuestController extends Controller
{
    ...
    public function getMetaData()
    {
        ...
        $karyas = DB::table("tb_karyas")
            ...
            ->select(
                ...
                "tb_karyas.judul",
                DB::raw(
                    '(SELECT content FROM tb_metadatas WHERE tb_metadatas.karyas_id = tb_karyas.id AND jenis = "description" LIMIT 1) as description'
                ),
                DB::raw(
                    '(SELECT content FROM tb_metadatas WHERE tb_metadatas.karyas_id = tb_karyas.id AND jenis = "imageTitle" ORDER BY `order` ASC LIMIT 1) as imageTitle'
                ),
            ...
            ->select(
                ...
                "tb_products.judul",
                DB::raw(
                    '(SELECT content FROM tb_products_items WHERE tb_products_items.products_id = tb_products.id AND jenis = "description" LIMIT 1) as description'
                ),
                DB::raw(
                    '(SELECT content FROM tb_products_items WHERE tb_products_items.products_id = tb_products.id AND jenis = "imageTitle" ORDER BY `order` ASC LIMIT 1) as imageTitle'
                ),
            ...
            ->select(
                ...
                "tb_events.judul",
                DB::raw(
                    '(SELECT content FROM tb_events_items WHERE tb_events_items.events_id = tb_events.id AND jenis = "description" LIMIT 1) as description'
                ),
                DB::raw(
                    '(SELECT content FROM tb_events_items WHERE tb_events_items.events_id = tb_events.id AND jenis = "imageTitle" ORDER BY `order` ASC LIMIT 1) as imageTitle'
                ),
        ...
        return view("home.pusatstudi", compact("karyas", "products", "events","carousel" ,"gallery", "team", "about"));
    }
    ...
    public function viewGuestEvent($id)
    {
        ...
        $eventItems = DB::table("tb_events_items")
            ->where("events_id", $id)
            ->orderBy("order")
            ->get();
    ...
    public function viewGuestProduct($id)
    {
        ...
        $productItems = DB::table("tb_products_items")
